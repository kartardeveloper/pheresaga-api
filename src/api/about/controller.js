...
module.exports = {
  createAbout: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ data: abouts, message: "About created successfully" });
    } catch (err) {
  ...
  getAllAbouts: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ data: abouts, message: "About retrieved successfully" });
    } catch (err) {
      return res.status(400).json({ error: err.messsage });
    }
  },
  ...
  updateAbout: async (req, res) => {
    try {
      const abouts = await About.findByIdAndUpdate(req.query.id, req.body, {
        new: true,
      });
      return res
        .status(200)
        .json({ data: abouts, message: "About updated successfully" });
    } catch (err) {
      return res.status(400).json({ error: err.messsage });
    }
  },
